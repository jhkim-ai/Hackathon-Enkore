      ...
      <div class="group_fundlist funding_list_box">
        ...
        <div class="search_box_outer">
          ...
          <v-text-field
            ...
            single-line
            hide-details
            @click:append="searchDonate"
          ></v-text-field>
          </div>
        </div>
        <ul class="list_fund_size list_fund fund_raising"><!---->
          <!--이거 통하나가 카드섹션 하나-->
          <li v-for="item in fetcheBoard" :key="item.board" class="listcard">
                <router-link class="p_card" :to="{ name: 'DonateDetail', query: { param1: getBoard(item), param2: '재단이름'} }" >
...
export default {
  ...
  computed: {
        ...mapGetters([
            'fetcheBoardCurInfo','fetcheBoard'
        ]),
  },
  components: {
    HomeCarousel,  
